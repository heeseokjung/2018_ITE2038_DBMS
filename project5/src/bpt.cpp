...
const int leaf_order = 3;
const int internal_order = 3;
...
// HW5 Locking find
int64_t* find(int table_id, int64_t key, int txid, int* result)
{
    if(cat_pointer[table_id] == NULL)
        return NULL;

    pthread_mutex_lock(&txn_mutex);
    // Request to aborted transaction
    ...
    while(true) {
        ...
        buf_node = buf_get_page(table_id, c);
        
        /* get buffer page latch and release buffer pool latch */
        pthread_mutex_lock(&buf_node->bufpage_latch);
        pthread_mutex_unlock(&bufcache_latch);
        /* acquire_lock */
        int lock_request = acquire_lock(table_id, txid, S_LOCK, c);
        if(lock_request == LOCK_GRANTED) {
            *result = 1;
            break;
        }
        else if(lock_request == LOCK_WAITING) {
            pthread_mutex_lock(&bufcache_latch);
            buf_put_page(buf_node, 0);
            pthread_mutex_unlock(&bufcache_latch);
            pthread_mutex_unlock(&buf_node->bufpage_latch);
            pthread_mutex_lock(&txn_mutex);
            auto tx = txn_table.find(txid);
            tx->second->txn_state = WAITING;
            pthread_cond_wait(&tx->second->txn_cond, &txn_mutex);
            pthread_mutex_unlock(&txn_mutex);
        }
        else if(lock_request == LOCK_DEADLOCK) {
            pthread_mutex_lock(&bufcache_latch);
            buf_put_page(buf_node, 0);
            pthread_mutex_unlock(&bufcache_latch);
            txn_abort(txid, result);
            pthread_mutex_unlock(&buf_node->bufpage_latch);
            return NULL;
        }
        else {
            printf("Invalid return value from lock manager.\n");
            exit(EXIT_FAILURE); 
        }
    }
...
int update(int table_id, int64_t key, int64_t *values, int txid, int* result)
{
    if(cat_pointer[table_id] == NULL)
        return -1;
    ...
    while(true) {
        ...
        pthread_mutex_unlock(&bufcache_latch);
        /* acquire_lock */
        int lock_request = acquire_lock(table_id, txid, X_LOCK, c);
        if(lock_request == LOCK_GRANTED) {
            *result = 1;
            break;
        }
    ...
    if(i == node->num_keys) {
        ...
        int col = cat_pointer[table_id]->num_column - 1;
        memcpy(node->record[i].values, values, sizeof(int64_t) * col);
        pthread_mutex_lock(&bufcache_latch);
...
int erase(int table_id, int64_t key)
{
    if(cat_pointer[table_id] == NULL)
        return -1;
    int64_t* value = find(table_id, key);
...
int init_db(int num_buf)
{
    ...
    for(int i = 1; i <= num_buf; ++i)
    {
        ...
        buf->is_dirty = 0;
        buf->pin_count = 0;
        buf->bufpage_latch = PTHREAD_MUTEX_INITIALIZER;
        buf->next = p->next;
        p->next = buf;
        buf->prev = p;
        p = buf;
